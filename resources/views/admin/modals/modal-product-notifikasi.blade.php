{{-- Modal Success Start --}}
<div x-data="{ open: false }" x-show="open" @product-success.window="open = true"
  @close-product-success.window="open = false" x-transition:enter="transition ease-out duration-300"
...
{{-- Modal Error Start --}}
<div x-data="{ open: false }" x-show="open" @product-error.window="open = true"
  @close-product-error.window="open = false" x-transition:enter="transition ease-out duration-300"
  x-transition:enter-start="opacity-0 transform scale-90" x-transition:enter-end="opacity-100 transform scale-100"
  x-transition:leave="transition ease-in duration-300" x-transition:leave-start="opacity-100 transform scale-100"
  x-transition:leave-end="opacity-0 transform scale-90"
  class="fixed inset-0 bg-gray-600 z-50 bg-opacity-50 overflow-y-auto h-full w-full" style="display: none;">
...
<div x-data="{ open: false }" x-show="open" @modal-confirm-delete.window="open = true"
  ...
  <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
    <div class="mt-3 text-center">
      ...
      <div class="mt-2">
        <img src="" alt="Image Delete" class="w-36 h-auto object-cover rounded-xl mx-auto">
        <p class="text-lg mt-2 text-black">"{{ $title }}"</p>
      </div>
      <div class="flex justify-between items-center px-4 py-3 gap-10">
        ...
        <button type="button" wire:click="resetProduct"
