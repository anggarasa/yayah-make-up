{{-- Modal Success Start --}}
<div x-data="{ open: false }" x-show="open" @diskon-success.window="open = true"
  @close-diskon-success.window="open = false" x-transition:enter="transition ease-out duration-300"
...
{{-- Modal Error Start --}}
<div x-data="{ open: false }" x-show="open" @diskon-error.window="open = true" @close-diskon-error.window="open = false"
...
<div x-data="{ open: false }" x-show="open" @modal-confirm-diskon.window="open = true"
  @close-modal-confirm-diskon.window="open = false" x-transition:enter="transition ease-out duration-300"
  x-transition:enter-start="opacity-0 transform scale-90" x-transition:enter-end="opacity-100 transform scale-100"
  x-transition:leave="transition ease-in duration-300" x-transition:leave-start="opacity-100 transform scale-100"
  x-transition:leave-end="opacity-0 transform scale-90"
  class="fixed inset-0 bg-gray-600 z-50 bg-opacity-50 overflow-y-auto h-full w-full" style="display: none;">
